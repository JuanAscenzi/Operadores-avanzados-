...
//porque yo alquilo los barriles asi y encima ponerles distinto precio porque el de 50 vale menos por llevar en cantidad,
// eso es una de las dudas que tengo a actualizar.
const canillas = [
...
//Si en el localStorage dice null en el lado izquierdo re vuelve al carrito vacio de la derecha en lo contrario devuelve lo recuperado del localStorage
...
//Con addEventListener logre que el click guarde en el storage la eleccion del carrito y lo traemos con el JSON.stringify 
listaProductosComprados.addEventListener("click",() => localStorage.setItem("carrito" , JSON.stringify(carrito)));
//Con este evento le doy funcion al buscador de canillas ya creado
botonInput.addEventListener("click",() => buscadorCanillas(inputAfter.value));
//Con este boton vacio el carrito y mando un alert 
botonVaciar.addEventListener("click", () => localStorage.clear(carrito))
botonVaciar.addEventListener("click", () => carrito = [] && alert("Vaciaste el carrito"))
